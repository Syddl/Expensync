...
export function WantVsNeed({displayData}) {
  const { needMonthly, wantMonthly, sortedExpense } = useFetchUserData()
  ...
  return (
    <PieChart
      series={[
        {
          data: [
            { id: 0, value: wantData, label: 'Wants', color: '#E63946' },
            { id: 1, value: needData, label: 'Needs', color: '#7f5efd' },
          ],
        },
      ]}
      width={1000} 
...
export function WantVsNeedSmall({displayData}) {
  const { needMonthly, wantMonthly, sortedExpense } = useFetchUserData()
  const [needData, setNeedData] = useState(needMonthly)
  const [wantData, setWantData] = useState(wantMonthly)
